import os, sys, shutil, threading, psutil, requests, subprocess, webbrowser, time, math, json, socket, urllib.parse, uuid
...
CONFIG_PATH = os.path.join(os.path.expanduser("~"), ".psp_media_suite_prefs.json")

# ── Dynamic Theme Engine ───────────────────────────────────────────────────────
BG = SURFACE = SURFACE2 = BORDER = ACCENT = ACCENT2 = TEXT = TEXT_DIM = SUCCESS = ERROR = WARN = ""

def set_theme(is_dark):
    global BG, SURFACE, SURFACE2, BORDER, ACCENT, ACCENT2, TEXT, TEXT_DIM, SUCCESS, ERROR, WARN
    if is_dark:
        BG       = "#121212"  
        SURFACE  = "#1e1e1e"  
        SURFACE2 = "#2d2d30"  
...
# ─── HTTP REQUEST HANDLER (PSP Web Interface) ─────────────────────────────────

class PSPRequestHandler(BaseHTTPRequestHandler):
    def log_message(self, format, *args):
        pass # Mute default HTTP logging to keep our GUI clean
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            if q:
                app.log(f"[PSP] Search requested: {q} (Page {page})")
                self._serve_results_page(q, page, app)
            else:
                self._serve_search_page()
                
        elif path == "/dl":
            dl_type = query.get('type', [''])[0]
            url = query.get('url', [''])[0]
            title = query.get('title', ['Unknown Video'])[0]
            thumb = query.get('thumb', [''])[0]
            
            task_id = uuid.uuid4().hex[:8]
            app.web_tasks[task_id] = {
                'status': 'processing',
            ...
            app.log(f"[PSP] Remote process started: {app.web_tasks[task_id]['title'][:30]}...")
            threading.Thread(target=app.process_web_download, args=(task_id, dl_type, url, urllib.parse.unquote_plus(thumb)), daemon=True).start()
            
            self.send_response(302)
            self.send_header('Location', f'/status?id={task_id}')
            self.end_headers()
            
        elif path == "/status":
            task_id = query.get('id', [''])[0]
            task = app.web_tasks.get(task_id)
            
            if not task:
                self._send_html("Task not found.")
                return
            ...
            if task['status'] == 'processing':
                ...
                <body bgcolor="#121212" text="#ffffff" style="font-family: sans-serif; text-align: center; padding-top: 50px;">
                    <h2 style="color: #f59e0b;">Please Wait</h2>
                    <p><b>{task['title'][:40]}...</b></p>
                    <p>Media is being Downloaded and converted...</p>
                    <p><font color="#9ca3af">This page will auto-refresh every 5 seconds.</font></p>
                </body></html>"""
                ...
                if task['type'] == 'audio':
                    ...
                    <head>
                        <title>Ready</title>
                        <meta http-equiv="refresh" content="0;url=/serve_file?id={task_id}">
                    </head>
                    <body bgcolor="#121212" text="#ffffff" style="font-family: sans-serif; text-align: center; padding-top: 50px;">
                        <h2 style="color: #22c55e;">Ready!</h2>
                        <p>Your file is ready. The download prompt should appear shortly.</p>
                        <p><a href="/serve_file?id={task_id}" style="color: #3b82f6;">Click here if it doesn't start</a></p>
                        <br><br><a href="/" style="color: #9ca3af;">&laquo; Back to Search</a>
                    </body></html>"""
                else:
                    # Video files require two separate downloads because PSP Browser can't handle Multi-Part
                    thm_btn = ""
                    if task.get('thm_file'):
                        thm_btn = f'<br><br><br><a href="/serve_file?id={task_id}&ext=thm" style="color: #ffffff; background-color: #f97316; padding: 10px; text-decoration: none; border-radius: 4px;">2. Download Thumbnail (.thm)</a>'
                    ...
                    <body bgcolor="#121212" text="#ffffff" style="font-family: sans-serif; text-align: center; padding-top: 40px;">
                        ...
                        {thm_btn}
                        <br><br><br><br><a href="/" style="color: #9ca3af;">&laquo; Back to Search</a>
                    </body></html>"""
                ...
                <body bgcolor="#121212" text="#ffffff" style="font-family: sans-serif; text-align: center; padding-top: 50px;">
                    ...
                    <p>Failed to process this item on the PC.</p>
                    <br><br><a href="/" style="color: #3b82f6;">&laquo; Back to Search</a>
                </body></html>"""
                self._send_html(html)
                
        elif path == "/serve_file":
            task_id = query.get('id', [''])[0]
            ext = query.get('ext', [''])[0]
            task = app.web_tasks.get(task_id)
            ...
            filepath = task.get('thm_file') if ext == 'thm' else task.get('file')
            ...
            try:
                with open(filepath, 'rb') as f:
                    ...
                    try:
                        ...
                        if "Broken pipe" not in str(stream_e) and "Connection reset" not in str(stream_e):
                            app.log(f"Stream note: {stream_e}")
            except Exception as e:
                if "Broken pipe" not in str(e) and "Connection reset" not in str(e):
                    app.log(f"Error streaming file to PSP: {e}")
    ...
    def _serve_search_page(self):
        html = """<html>
        <head><title>PSP Media Suite</title></head>
        <body bgcolor="#121212" text="#ffffff" style="font-family: sans-serif;">
            <h2 style="color: #3b82f6;">PSP Media Search</h2>
            <form action="/search" method="get">
                <input type="text" name="q" size="30" style="padding: 4px;">
                <input type="submit" value="Search YouTube">
            </form>
        </body></html>"""
        self._send_html(html)

    def _serve_results_page(self, query_str, page, app):
        opts = {'quiet': True, 'extract_flat': True}
        if app.selected_browser and app.selected_browser != "None":
            try: opts['cookiesfrombrowser'] = (app.selected_browser.lower(),)
            except: pass
        ...
        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                info = ydl.extract_info(f"ytsearch{fetch_count}:{query_str}", download=False)
                results = list(info.get('entries', []))
        except Exception as e:
            app.log(f"Search error: {e}")

        display_results = results[(page - 1) * 10 : fetch_count]

        html = f"""<html>
        <head><title>Results for {query_str}</title></head>
        <body bgcolor="#121212" text="#ffffff" style="font-family: sans-serif; font-size: 12px;">
            <h3>Results for "{query_str}"</h3>
            <a href="/" style="color: #3b82f6;">&laquo; Back to Search</a>
            <hr color="#3f3f46">
            <table width="100%" cellpadding="6" cellspacing="0" border="1" bordercolor="#3f3f46">"""
        ...
        for r in display_results:
            ...
            duration = app.format_time(r.get('duration', 0))
            
            thumb = r.get('thumbnails', [{'url': ''}])[-1].get('url', '')
            
            safe_url = urllib.parse.quote(url)
            safe_title = urllib.parse.quote(title)
            safe_thumb = urllib.parse.quote(thumb)

            html += f"""
            <tr>
                <td valign="top">
                    <b>{title[:55]}...</b><br>
                    <font color="#9ca3af">{channel}  ·  {duration}</font>
                </td>
                <td width="110" align="center" valign="middle">
                    <a href="/dl?type=audio&url={safe_url}&title={safe_title}&thumb={safe_thumb}" style="color: #22c55e; font-weight: bold; text-decoration: none;">[ AUDIO ]</a><br><br>
                    <a href="/dl?type=video&url={safe_url}&title={safe_title}&thumb={safe_thumb}" style="color: #f97316; font-weight: bold; text-decoration: none;">[ VIDEO ]</a>
                </td>
            </tr>"""
        ...
        <div align="center" style="padding: 10px;">
            <a href="/search?q={safe_query}&page={page + 1}" style="color: #ffffff; font-size: 14px; font-weight: bold; background-color: #3b82f6; text-decoration: none; padding: 6px 12px; border: 1px solid #3b82f6;">&nbsp;Load More Results&nbsp;</a>
        </div>
        <br>
        </body></html>"""
        self._send_html(html)

class PSPServerConfig(HTTPServer):
...
class PSPMediaSuite:
    def __init__(self, root):
        ...
        try:
            ...
            except: self.root.geometry("1280x720")
        ...
        if os.path.exists(CONFIG_PATH):
            try:
                with open(CONFIG_PATH, "r") as f:
                    data = json.load(f)
                    self.selected_browser = data.get("browser", "None")
    ...
    def _save_prefs(self):
        try:
            ...
            self.write_log(f"Failed to save settings: {ex}")
    ...
    def _build_ui(self):
        ...
        style.configure("App.TCombobox", fieldbackground=SURFACE2, background=SURFACE2,
                        foreground=TEXT, selectforeground=TEXT, selectbackground=SURFACE2, 
                        borderwidth=1, arrowcolor=ACCENT, insertcolor=TEXT, relief="flat")
        style.map("App.TCombobox", fieldbackground=[('readonly', SURFACE2)],
                  background=[('readonly', SURFACE2)], foreground=[('readonly', TEXT)])
        ...
        # Banner
        self.banner_canvas = tk.Canvas(self.root, bg="#121212", highlightthickness=0, height=68)
        self.banner_canvas.pack(fill="x")

        right_bar = tk.Frame(self.banner_canvas, bg="#121212")
        right_bar.place(relx=1.0, rely=0.5, anchor="e", x=-16)

        about_btn = self._flat_btn(right_bar, "☰", bg="#2d2d30", fg="#ffffff", hover_bg="#3f3f46",
                                   command=self.show_about)
        about_btn.pack(side="right", padx=(8, 0))

        self.server_btn = self._flat_btn(right_bar, "🌐 Start Server", bg="#2d2d30", fg="#ffffff",
                                         hover_bg="#3f3f46", command=self.toggle_server)
        self.server_btn.pack(side="right", padx=(8, 0))

        refresh_btn = self._flat_btn(right_bar, "⟳  Refresh", bg="#2d2d30", fg="#ffffff",
                                     hover_bg="#3f3f46", command=self.scan_usb)
        refresh_btn.pack(side="right", padx=(8, 0))

        self.drive_combo = ttk.Combobox(right_bar, state="readonly", style="App.TCombobox", width=28, font=FONT_BODY)
        self.drive_combo.pack(side="right")

        drive_lbl = tk.Label(right_bar, text="Drive:", font=FONT_SMALL, fg="#9ca3af", bg="#121212")
        ...
        q_title = tk.Label(q_hdr, text="Local Transfer Queue", font=("Segoe UI", 11, "bold"), fg=TEXT, bg=BG)
        ...
        log_lbl_txt = tk.Label(log_label, text=" ◉  Activity Log", font=("Segoe UI", 8, "bold"), fg=ACCENT, bg=SURFACE2)
        ...
        self._prog_label = tk.Label(prog_area, text="Ready", font=("Segoe UI", 9), fg=TEXT_DIM, bg=SURFACE, anchor="w")
        ...
        self.send_btn = self._accent_button(inner_footer, "⬆  SEND QUEUE VIA USB",
                                            width=220, height=50,
    ...
    def toggle_server(self):
        if self.server:
            ...
            self.server_btn.config(text="🌐 Start Server", fg="#ffffff")
            self.write_log("Web Server stopped.")
        else:
            ip = get_local_ip()
            def run():
                self.server = PSPServerConfig(("0.0.0.0", self.port), PSPRequestHandler, self)
                self.server.serve_forever()

            self.server_thread = threading.Thread(target=run, daemon=True)
            self.server_thread.start()
            
            self.server_btn.config(text="⏹ Stop Server", fg="#f59e0b")
            self.write_log(f"Server started! Navigate PSP to: http://{ip}:{self.port}")
            self._show_server_started_dialog(ip)

    def _show_server_started_dialog(self, ip):
        pop = tk.Toplevel(self.root)
        pop.title("Server Started")
        ...
        msg = f"Navigate your PSP browser to:\n\nhttp://{ip}:{self.port}"
        tk.Label(pop, text=msg, font=("Segoe UI", 11, "bold"), bg=SURFACE, fg=TEXT).pack(pady=25)

        btn_container = tk.Frame(pop, bg=SURFACE)
        btn_container.pack(fill="x", side="bottom", pady=15)

        self._flat_btn(btn_container, "More Info", bg=SURFACE2, fg=ACCENT, hover_bg=BORDER, 
                       command=self._show_web_server_info).pack(pady=(0, 8))
        self._flat_btn(btn_container, "Okay", bg=ACCENT, fg="#ffffff", hover_bg=ACCENT2, 
                       command=pop.destroy).pack()

    def _show_web_server_info(self):
        win = tk.Toplevel(self.root)
        win.title("Web Server Info")
        ...
        txt = tk.Text(txt_frame, wrap="word", bg=SURFACE, fg=TEXT, font=FONT_MONO,
        ...
        txt.insert(tk.END, "Loading info from GitHub...\n")
        win.update()
        
        def fetch():
            try:
                url = "https://raw.githubusercontent.com/vmg265/PSP-Media-Suite/main/web-server-info.txt"
                r = requests.get(url, timeout=5)
                r.raise_for_status()
                content = r.text
            except Exception as e:
                content = f"Failed to load info from GitHub.\n\nError: {e}\n\nPlease check your internet connection."
            self.root.after(0, lambda: _update_txt(content))
            
        def _update_txt(content):
    ...
    # ─── DIRECT WEB DOWNLOAD LOGIC ──────────────────────────

    def process_web_download(self, task_id, dl_type, url, thumb_url):
        task = self.web_tasks[task_id]
        clean_title = task['title']
        clean_name  = "".join(x for x in clean_title if x.isalnum() or x in " .-_")[:80]
        
        out_dir = os.path.join(get_resource_path(""), "web_out")
        ...
        raw_dl = f"temp_raw_{task_id}.%(ext)s"
        
        try:
            if dl_type == 'audio':
                opts = {
                    'format': 'bestaudio/best', 'ffmpeg_location': ff_path,
                    'outtmpl': raw_dl, 'nopart': True, 'continuedl': False,
                    'postprocessors': [{'key': 'FFmpegExtractAudio',
                                       'preferredcodec': 'mp3', 'preferredquality': '192'}],
                    'postprocessor_args': ['-map_metadata', '-1'],
                    'quiet': True
                }
                if self.selected_browser and self.selected_browser != "None":
                    try: opts['cookiesfrombrowser'] = (self.selected_browser.lower(),)
                    except: pass
                ...
                img = self._get_best_thumbnail(info_dict, thumb_url)
                cover_path = f"cover_{task_id}.jpg"
                if img:
                    try:
                ...
                dl_file = f"temp_raw_{task_id}.mp3"
                if os.path.exists(dl_file):
                    audio = MP3(dl_file, ID3=ID3)
                    ...
                    if os.path.exists(cover_path):
                        with open(cover_path, "rb") as af:
                            audio.tags.add(APIC(encoding=3, mime='image/jpeg', type=3, desc='Cover', data=af.read()))
                            
                    audio.save(v2_version=3, v1=2)
                    
                    final_path = os.path.join(out_dir, f"{clean_name}.mp3")
                    if os.path.exists(final_path): os.remove(final_path)
                    shutil.move(dl_file, final_path)
                    
                    task['file'] = final_path
                    task['status'] = 'done'
                    self.log(f"✔ Web Task Ready: {clean_name}.mp3")
                else:
                ...
                opts = {
                    ...
                    'ffmpeg_location': ff_path, 'outtmpl': raw_dl,
                    'nopart': True, 'continuedl': False, 'quiet': True
                }
                if self.selected_browser and self.selected_browser != "None":
                    try: opts['cookiesfrombrowser'] = (self.selected_browser.lower(),)
                    except: pass
                    
                self.log("Downloading video from YouTube...")
                with yt_dlp.YoutubeDL(opts) as ydl:
                    info_dict = ydl.extract_info(url, download=True)
                
                raw_file = next((f for f in os.listdir(".") if f.startswith(f"temp_raw_{task_id}") and not f.endswith(".mp3")), None)
                if raw_file:
                    final_path = os.path.join(out_dir, f"{clean_name}.mp4")
                    if os.path.exists(final_path): os.remove(final_path)
                    subprocess.run([ff_path, "-y", "-i", raw_file,
                                    "-c:v", "libx264", "-profile:v", "baseline", "-level", "3.0",
                                    "-pix_fmt", "yuv420p", "-vf", "scale=480:272",
                                    "-b:v", "768k", "-c:a", "aac", "-ac", "2", "-b:a", "128k",
                                    "-ar", "48000", final_path], startupinfo=startupinfo)
                    task['file'] = final_path
                    ...
                    if img:
                        try:
                            ...
                            thm_path = os.path.join(out_dir, f"{clean_name}.thm")
                            img.resize((160, 120), Image.Resampling.LANCZOS).convert('RGB').save(thm_path, "JPEG")
                            task['thm_file'] = thm_path
                        except Exception as e:
                            self.log(f".THM Error: {str(e)[:40]}")
                            
                    task['status'] = 'done'
                    self.log(f"✔ Web Task Ready: {clean_name}.mp4")
                else:
                    raise Exception("Video download failed")

        except Exception as e:
            self.log(f"✕ Web Processing Error: {e}")
            task['status'] = 'error'

        finally:
            for f in os.listdir("."):
                if task_id in f and (f.startswith("temp_raw") or f.startswith("cover")):
                    try: os.remove(f)
                    except: pass

    # ─── GUI ELEMENTS AND MANAGERS ────────────────────────────────────────────────
    ...
    def open_file_manager(self, media_type):
        ...
        style = ttk.Style()
        style.configure("Manager.Treeview", background=SURFACE, fieldbackground=SURFACE,
                        foreground=TEXT, rowheight=26, borderwidth=0)
        style.configure("Manager.Treeview.Heading", background=SURFACE2, foreground=TEXT, borderwidth=0)
        style.map("Manager.Treeview", background=[("selected", ACCENT)], foreground=[("selected", "#ffffff")])
        ...
        def delete_selected():
            sel = tree.selection()
            if not sel: return
            if not messagebox.askyesno("Confirm Delete", f"Delete {len(sel)} file(s)?", parent=win): return
            for fname in sel:
                try:
                    os.remove(os.path.join(folder, fname))
                    if media_type == 'video':
                        thm_path = os.path.join(folder, os.path.splitext(fname)[0] + ".thm")
                        if os.path.exists(thm_path):
                            try: os.remove(thm_path)
                            except: pass
                except Exception as e:
                    messagebox.showerror("Error", str(e), parent=win)
            load_files()
        ...
        for txt, cmd, col in [("✏  Rename", rename_selected, ACCENT), ("🗑  Delete", delete_selected, ERROR),
                              ("📂  Open Folder", open_folder, SURFACE2), ("↩  Refresh", load_files, SURFACE2)]:
    ...
    def open_playlist_manager(self):
        ...
        tk.Label(hdr, text="🎶  Playlist Manager", font=("Segoe UI", 13, "bold"), fg=TEXT, bg=BG).pack(side="left")
        ...
        tk.Label(left_pane, text="Playlists", font=("Segoe UI", 10, "bold"), fg=TEXT, bg=BG).pack(anchor="w", pady=(0,4))
        ...
        tk.Label(right_pane, textvariable=pl_name_var, font=("Segoe UI", 10, "bold"), fg=TEXT, bg=BG).pack(anchor="w", pady=(0,4))
        ...
        song_listbox = tk.Listbox(song_outer, bg=SURFACE, fg=TEXT, font=("Segoe UI", 10),
                                  selectbackground=ACCENT, selectforeground="#ffffff",
                                  borderwidth=0, highlightthickness=0, activestyle="none", selectmode="extended")
        ...
        def _pl_path(name): return os.path.join(pl_dir, name + ".m3u8")
        def _read_pl(name):
            path = _pl_path(name)
            if not os.path.exists(path): return []
            with open(path, "r", encoding="utf-8") as f: return [l.strip() for l in f if l.strip()]
        def _write_pl(name, lines):
            with open(_pl_path(name), "w", encoding="utf-8") as f: f.write("\n".join(lines))
        def _song_display(raw_line): return os.path.basename(raw_line.replace("\\", "/"))

        def load_playlists():
            pl_listbox.delete(0, tk.END)
            try:
                for f in sorted(os.listdir(pl_dir)):
                    if f.lower().endswith(".m3u8"): pl_listbox.insert(tk.END, os.path.splitext(f)[0])
            except Exception: pass
        ...
        def load_songs():
            ...
            for line in _read_pl(current_pl["name"]): song_listbox.insert(tk.END, _song_display(line))
        ...
        def delete_playlist():
            if not current_pl["name"]: return
            if not messagebox.askyesno("Delete", f"Delete playlist '{current_pl['name']}'?", parent=win): return
            try:
        ...
        for txt, cmd, col in [("＋ New", new_playlist, ACCENT), ("✏ Rename", rename_playlist, SURFACE2), ("🗑 Delete", delete_playlist, ERROR)]:
            b = self._flat_btn(pl_btn_row, txt, bg=col, fg="#ffffff" if col != SURFACE2 else TEXT,
                               hover_bg=BORDER, command=cmd, font=("Segoe UI", 9, "bold"))
            b.pack(side="top", fill="x", pady=3)

        def add_songs():
            if not current_pl["name"]: return
            try: avail = [f for f in sorted(os.listdir(music_dir)) if f.lower().endswith(".mp3")]
            except: avail = []
            if not avail:
            ...
            tk.Label(pick, text="Select songs to add:", font=("Segoe UI", 10, "bold"), fg=TEXT, bg=BG).pack(padx=16, pady=(12,4), anchor="w")
            ...
            pick_lb = tk.Listbox(pick_outer, bg=SURFACE, fg=TEXT, font=("Segoe UI", 10),
                                 selectbackground=ACCENT, selectforeground="#ffffff", borderwidth=0, highlightthickness=0, selectmode="extended")
            ...
            for f in avail:
                if f.lower() not in existing_files: pick_lb.insert(tk.END, f)

            def confirm_add():
                sel = pick_lb.curselection()
                if not sel: pick.destroy(); return
                lines = _read_pl(current_pl["name"])
                for idx in sel: lines.append(f"\\MUSIC\\{pick_lb.get(idx)}")
            ...
            self._flat_btn(add_row, "✚  Add Selected", bg=ACCENT, fg="#ffffff", hover_bg=ACCENT2, command=confirm_add, font=("Segoe UI", 10, "bold")).pack(side="left")
            self._flat_btn(add_row, "Cancel", bg=SURFACE2, fg=TEXT, hover_bg=BORDER, command=pick.destroy).pack(side="left", padx=(8,0))
        ...
        def move_up():
            ...
            lines = _read_pl(current_pl["name"])
            for i in sel:
                if i > 0: lines[i-1], lines[i] = lines[i], lines[i-1]
            _write_pl(current_pl["name"], lines)
            load_songs()
        ...
        def move_down():
            ...
            for i in reversed(sel):
                if i < len(lines)-1: lines[i], lines[i+1] = lines[i+1], lines[i]
            _write_pl(current_pl["name"], lines)
            load_songs()
            song_listbox.selection_set([i+1 for i in sel if i < len(lines)-1])

        for txt, cmd, col in [("✚ Add Songs", add_songs, ACCENT), ("✕ Remove", remove_songs, ERROR),
                              ("▲ Move Up", move_up, SURFACE2), ("▼ Move Down", move_down, SURFACE2)]:
            b = self._flat_btn(song_btn_row, txt, bg=col, fg="#ffffff" if col not in (SURFACE2,) else TEXT,
                               hover_bg=BORDER, command=cmd, font=("Segoe UI", 9, "bold"))
    ...
    def show_about(self):
        ...
        for label, url in [("⭐  GitHub", "https://github.com/vmg265/PSP-Media-Suite"),
            ...
            b = self._flat_btn(inner, label, bg=SURFACE2, fg=TEXT, hover_bg=BORDER, command=lambda u=url: webbrowser.open(u), font=("Segoe UI", 10))
            b.pack(fill="x", pady=3)

        self._flat_btn(inner, "🔧  Troubleshooter", bg=SURFACE2, fg=ACCENT, hover_bg=BORDER, command=self.show_troubleshooter, font=("Segoe UI", 10)).pack(fill="x", pady=(12, 0))

        tk.Frame(inner, bg=BORDER, height=1).pack(fill="x", pady=12)
        tk.Label(inner, text="Appearance", font=("Segoe UI", 9, "bold"), fg=TEXT, bg=SURFACE).pack(anchor="w")
        ...
        def on_theme_toggle():
            ...
            self._save_prefs()
            messagebox.showinfo("Theme Changed", "Please restart the application to apply the new theme.", parent=about)
            
        chk = tk.Checkbutton(inner, text="Dark Mode", variable=self.dark_mode_var, bg=SURFACE, fg=TEXT, selectcolor=SURFACE2,
                             activebackground=SURFACE, activeforeground=TEXT, command=on_theme_toggle, cursor="hand2")
        chk.pack(anchor="w", pady=(4, 0))

        tk.Label(inner, text="Bypass YouTube Bot/Age Checks", font=("Segoe UI", 9, "bold"), fg=TEXT, bg=SURFACE).pack(anchor="w", pady=(10, 0))
        tk.Label(inner, text="Select the browser where you are logged into YouTube:", font=("Segoe UI", 8), fg=TEXT_DIM, bg=SURFACE, wraplength=250, justify="left").pack(anchor="w", pady=(2, 6))

        menu_cookie_combo = ttk.Combobox(inner, state="readonly", style="App.TCombobox", font=FONT_BODY)
        menu_cookie_combo['values'] = ["None", "Chrome", "Edge", "Firefox", "Brave", "Safari", "Opera"]
        menu_cookie_combo.set(self.selected_browser)
        menu_cookie_combo.pack(fill="x")

        def on_browser_change(e):
            self.selected_browser = menu_cookie_combo.get()
            self._save_prefs()

        menu_cookie_combo.bind("<<ComboboxSelected>>", on_browser_change)
    ...
    def show_troubleshooter(self):
        ...
        win.configure(bg=SURFACE)
        tk.Frame(win, bg=ACCENT, height=2).pack(fill="x")
        txt = tk.Text(win, wrap="word", bg=SURFACE, fg=TEXT, font=FONT_MONO, padx=12, pady=12, insertbackground=ACCENT, selectbackground=ACCENT2, borderwidth=0)
        txt.pack(fill="both", expand=True, padx=8, pady=8)
        
        txt.insert(tk.END, "Loading troubleshooter data from GitHub...\n\n")
        win.update()
        
        try:
            url = "https://raw.githubusercontent.com/vmg265/PSP-Media-Suite/main/troubleshooter_box.txt"
            response = requests.get(url, timeout=5)
            if response.status_code == 200:
                txt.delete(1.0, tk.END)
                txt.insert(tk.END, response.text)
            else:
                txt.insert(tk.END, f"Failed to load. HTTP Status: {response.status_code}")
        except Exception as e:
            txt.insert(tk.END, f"Error reaching GitHub:\n{e}\n\nPlease check your internet connection.")
            
        txt.config(state="disabled")

    def _flat_btn(self, parent, text, width=None, height=32, bg=None, fg=None, hover_bg=None, command=None, font=None):
        bg = bg or SURFACE2
        fg = fg or TEXT
        hover_bg = hover_bg or BORDER
        f = font or FONT_BTN
        btn = tk.Label(parent, text=text, font=f, fg=fg, bg=bg, anchor="center", cursor="hand2", padx=10, pady=4)
        btn._bg = bg
        btn._hbg = hover_bg
        btn.bind("<Enter>", lambda e: btn.config(bg=btn._hbg))
        btn.bind("<Leave>", lambda e: btn.config(bg=btn._bg))
        if command: btn.bind("<Button-1>", lambda e: command())
        return btn

    def _tab_button(self, parent, text, tab_id):
        btn = tk.Label(parent, text=text, font=FONT_TAB, fg=TEXT_DIM, bg=SURFACE2, padx=16, pady=8, cursor="hand2")
    ...
    def _accent_button(self, parent, text, width=180, height=44, command=None):
        canvas = tk.Canvas(parent, bg=SURFACE, highlightthickness=0, width=width, height=height, cursor="hand2")
        canvas._text = text
        canvas._state = "normal"
        self._draw_accent_btn(canvas, width, height, text)
        if command: canvas.bind("<Button-1>", lambda e: command())
    ...
    def _draw_accent_btn(self, canvas, w, h, text, hover=False):
        ...
        if state == "disabled": fill = SURFACE2; txt_color = TEXT_DIM
        elif state == "processing": fill = WARN; txt_color = "#000000"
        elif hover: fill = ACCENT2; txt_color = "#ffffff"
        else: fill = ACCENT; txt_color = "#ffffff"
        self._rrect(canvas, 0, 0, w, h, 8, fill)
        canvas.create_text(w//2, h//2, text=text, font=("Segoe UI", 11, "bold"), fill=txt_color, anchor="center")
    ...
    def switch_tab(self, tab_id):
        ...
        for frame in [self.tab_music, self.tab_video, self.tab_playlist]: frame.pack_forget()
        {"audio": self.tab_music, "video": self.tab_video, "playlist": self.tab_playlist}[tab_id].pack(fill="both", expand=True)
        for tid, btn in self._tab_btns.items():
            if tid == tab_id: btn.config(bg=ACCENT, fg="#ffffff")
            else: btn.config(bg=SURFACE2, fg=TEXT_DIM)

    def setup_tab(self, parent, media_type):
        search_row = tk.Frame(parent, bg=BG)
        search_row.pack(fill="x", pady=(0, 8))

        search_frame = tk.Frame(search_row, bg=SURFACE2, highlightbackground=BORDER, highlightthickness=1)
        search_frame.pack(side="left", fill="x", expand=True)

        tk.Label(search_frame, text="🔍", font=("Segoe UI", 11), fg=TEXT_DIM, bg=SURFACE2, padx=8).pack(side="left")

        search_var = tk.StringVar()
        entry = tk.Entry(search_frame, textvariable=search_var, fg=TEXT_DIM, font=FONT_BODY, borderwidth=0, relief="flat", bg=SURFACE2, insertbackground=TEXT, width=40)
        ...
        self._flat_btn(search_row, "Search", bg=ACCENT, fg="#ffffff", hover_bg=ACCENT2, command=lambda: self.search(search_var.get(), parent, media_type), font=("Segoe UI", 10, "bold")).pack(side="left", padx=(8, 0))

        if media_type != "playlist":
            btn_row = tk.Frame(parent, bg=BG)
            btn_row.pack(fill="x", pady=(0, 8))
            self._flat_btn(btn_row, "📁  Add Local Music" if media_type == "audio" else "📁  Add Local Video", bg=SURFACE2, fg=TEXT, hover_bg=BORDER, command=lambda: self.add_local_file(media_type)).pack(side="left", fill="x", expand=True, padx=(0, 6))
            self._flat_btn(btn_row, "🗂  Manage Music" if media_type == "audio" else "🗂  Manage Videos", bg=SURFACE2, fg=ACCENT, hover_bg=BORDER, command=lambda mt=media_type: self.open_file_manager(mt), font=("Segoe UI", 9, "bold")).pack(side="left", fill="x", expand=True)
        else:
            pl_btn_row = tk.Frame(parent, bg=BG)
            pl_btn_row.pack(fill="x", pady=(0, 8))
            self._flat_btn(pl_btn_row, "🎶  Manage Playlists", bg=SURFACE2, fg=ACCENT, hover_bg=BORDER, command=self.open_playlist_manager, font=("Segoe UI", 9, "bold")).pack(fill="x")
        ...
        results_canvas.bind("<Configure>", lambda e, c=results_canvas, w=parent.res_window: c.itemconfig(w, width=e.width))
    ...
    def _generate_gradient(self):
        ...
        for x in range(size):
            ...
            base.putpixel((x, 0), tuple(int(c1[i] + (c2[i] - c1[i]) * blend) for i in range(3)))
        try:
        ...
        self._master_gradient = base.resize((size, size), resize_filter).rotate(25, resample=rotate_filter, expand=True)
    ...
    def _draw_banner(self):
        ...
        self._banner_img_ref = ImageTk.PhotoImage(img)
        c.delete("gradient")
        c.create_image(0, 0, anchor="nw", image=self._banner_img_ref, tags="gradient")
        ...
        if not hasattr(self, '_banner_icon'):
            ...
            if os.path.exists(get_resource_path("Icon.png")):
                try: self._banner_icon = ImageTk.PhotoImage(Image.open(get_resource_path("Icon.png")).resize((40, 40), Image.Resampling.LANCZOS))
                except: pass
        tx = 20
        if self._banner_icon:
            c.create_image(20, h//2, anchor="w", image=self._banner_icon, tags="banner_icon")
            tx = 72
        c.create_text(tx, h//2, text="PSP Media Suite", font=("Segoe UI", 20, "bold"), fill="#ffffff", anchor="w", tags="banner_text")

    def _animate(self):
        self.pulse_phase = (self.pulse_phase + 0.08) % (2 * math.pi)
        self.gradient_offset = (self.gradient_offset + 1) % 1500
        self._draw_banner()
        if self.current_progress > 0: self._draw_progress()
        self.root.after(40, self._animate)
    ...
    def _draw_progress(self):
        ...
        if self.current_progress > 0:
            self._rrect(c, 0, 0, max(r*2, int(w * self.current_progress / 100)), h, r, self._pulse_color(ACCENT, "#0055aa"))
    ...
    def update_progress(self, val=None):
        if val is not None:
            ...
            elif val == 0: self._prog_label.config(text="Ready", fg=TEXT_DIM)
            else: self._prog_label.config(text=f"Processing… {int(val)}%", fg=ACCENT)
        self._draw_progress()
    ...
    def _get_drive_path(self): return self.drives.get(self.drive_combo.get())
    ...
    def add_local_file(self, media_type):
        ft = [("Audio Files", "*.mp3 *.ogg *.opus *.m4a *.wav")] if media_type == "audio" else [("Video Files", "*.mp4 *.mkv *.avi *.mov *.webm")]
        for path in filedialog.askopenfilenames(title=f"Select Local {media_type.capitalize()}", filetypes=ft):
            name_no_ext = os.path.splitext(os.path.basename(path))[0]
            self.add_to_queue({'title': f"(Local) {name_no_ext}", 'url': path, 'is_local': True, 'pil_image': None, 'formatted_time': 'Local', 'raw_thumb_url': None}, media_type)
    ...
    def _search_thread(self, q, parent, media_type):
        ...
        if self.selected_browser and self.selected_browser != "None":
            try: opts['cookiesfrombrowser'] = (self.selected_browser.lower(),)
            except: self.write_log("Could not load browser cookies. Falling back to default.")

        try:
            with yt_dlp.YoutubeDL(opts) as ydl:
                if q.startswith("http"):
                    info = ydl.extract_info(q, download=False)
                    parent.cached_results = [info] if info.get('_type') == 'playlist' or 'entries' not in info else list(info['entries'])
                else:
                    parent.cached_results = list(ydl.extract_info(f"ytsearch24:{q}", download=False).get('entries', []))
            self.root.after(0, self.trigger_load_chunk, parent, media_type)
        except Exception as e:
            self.root.after(0, lambda: self.update_progress(0))
            self.root.after(0, lambda e_msg=str(e): self.write_log(f"Search failed: {e_msg}"))
    ...
    def _load_chunk_thread(self, parent, media_type):
        ...
        for r in chunk:
            ...
            if not url:
                vid_id = str(r.get('id', ''))
                url = f"https://www.youtube.com/playlist?list={vid_id}" if r.get('_type') == 'playlist' or vid_id.startswith(('PL','OL','RD')) else f"https://www.youtube.com/watch?v={vid_id}"
                if "watch?v=PL" in url or "watch?v=OL" in url: url = url.replace("watch?v=", "playlist?list=")
            r['url'] = url
    ...
    def fetch_image_bytes(self, url):
        try: return Image.open(BytesIO(requests.get(url, timeout=3).content)).resize((80, 45))
        except: return None

    def render_chunk(self, chunk, parent, media_type):
        btn_map = {"audio": ("+ Add Music", ACCENT, "#ffffff"), "video": ("+ Add Video", ACCENT2, "#ffffff"), "playlist": ("+ Add Playlist", "#8b5cf6", "#ffffff")}
        ...
        for item in chunk:
            ...
            else: thumb_lbl.config(bg=SURFACE2, text="▶", fg=TEXT_DIM, font=("Segoe UI", 14))
            ...
            tk.Label(info_f, text=item.get('title','Unknown')[:65], bg=SURFACE, fg=TEXT, font=FONT_BODY, anchor="w").pack(fill="x")
            tk.Label(info_f, text=item['formatted_time'], bg=SURFACE, fg=TEXT_DIM, font=FONT_SMALL, anchor="w").pack(fill="x")
    ...
    def add_to_queue(self, item, media_type):
        now = time.time()
        if hasattr(self, '_last_add_url') and self._last_add_url == item['url'] and now - getattr(self, '_last_add_time', 0) < 0.6: return
        self._last_add_url = item['url']
        self._last_add_time = now

        icon = {"audio": "🎵", "video": "🎬", "playlist": "🎶"}.get(media_type, "•")
        ...
        else: tk.Label(thumb_area, text=icon, font=("Segoe UI", 18), fg=TEXT_DIM, bg=SURFACE2).place(relx=.5, rely=.55, anchor="center")

        rm_btn = tk.Label(row, text="✕", font=("Segoe UI", 10), fg=TEXT_DIM, bg=SURFACE2, cursor="hand2", padx=6)
        ...
        tk.Label(info, text=item['title'][:42], bg=SURFACE2, fg=TEXT, font=("Segoe UI", 9), anchor="w").pack(fill="x")
        tk.Label(info, text=f"{icon}  {media_type.upper()}  ·  {item['formatted_time']}", bg=SURFACE2, fg=TEXT_DIM, font=("Segoe UI", 8), anchor="w").pack(fill="x")

        q_data = {'status': 'pending', 'type': media_type, 'url': item['url'], 'title': item['title'], 'thumb': item.get('raw_thumb_url'), 'is_local': item.get('is_local', False), 'status_cvs': status_canvas, 'q_row': row}
    ...
    def _draw_status(self, canvas, status, text=""):
        canvas.delete("all")
        bg, fg, sym = {"pending": (SURFACE,TEXT_DIM,"·"), "processing": (WARN,"#000","⏳"), "success": (SUCCESS,"#000","✓"), "error": (ERROR,"#fff","✕")}.get(status, (SURFACE, TEXT_DIM, "·"))
        if text: sym = text
    ...
    def _set_send_btn(self, text, state="normal"):
        ...
        self._draw_accent_btn(self.send_btn, self.send_btn.winfo_width() or 220, self.send_btn.winfo_height() or 50, text)
    ...
    def _process_queue(self, drive_path):
        ...
        for i, item in enumerate(self.queue):
            ...
            clean_name  = "".join(x for x in (os.path.splitext(item['title'])[0] if item.get('is_local') else item['title']) if x.isalnum() or x in " .-_")[:100]
            ...
            try:
                if item['type'] == 'playlist':
                    target_dir, playlist_dir = os.path.join(drive_path, "MUSIC"), os.path.join(drive_path, "PSP", "PLAYLIST", "MUSIC")
                    os.makedirs(target_dir, exist_ok=True); os.makedirs(playlist_dir, exist_ok=True)
                    m3u8_lines = []
                    opts = {'quiet': True, 'extract_flat': True}
                    if self.selected_browser and self.selected_browser != "None":
                        try: opts['cookiesfrombrowser'] = (self.selected_browser.lower(),)
                    ...
                    with yt_dlp.YoutubeDL(opts) as ydl: entries = ydl.extract_info(item['url'], download=False).get('entries', [])
                    for ei, entry in enumerate(entries):
                        if not entry: continue
                        entry_url, entry_title = entry.get('url') or f"https://www.youtube.com/watch?v={entry.get('id')}", entry.get('title', f"Track {ei+1}")
                        ...
                        dl_opts = {'format': 'bestaudio/best', 'ffmpeg_location': ff_path, 'outtmpl': 'temp_raw.%(ext)s', 'nopart': True, 'continuedl': False, 'postprocessors': [{'key': 'FFmpegExtractAudio', 'preferredcodec': 'mp3', 'preferredquality': '192'}], 'postprocessor_args': ['-map_metadata', '-1']}
                        if self.selected_browser and self.selected_browser != "None":
                            try: dl_opts['cookiesfrombrowser'] = (self.selected_browser.lower(),)
                            except: pass
                            
                        try:
                            with yt_dlp.YoutubeDL(dl_opts) as ydl2: einfo = ydl2.extract_info(entry_url, download=True)
                            img = self._get_best_thumbnail(einfo, entry.get('thumbnail'))
                            if img:
                                try:
                                    w2, h2 = img.size; s = min(w2, h2)
                                    img.crop(((w2-s)//2, (h2-s)//2, (w2+s)//2, (h2+s)//2)).resize((600, 600), Image.Resampling.LANCZOS).convert('RGB').save("cover.jpg", "JPEG", quality=85)
                                except: pass
                            ...
                            if os.path.exists("cover.jpg"):
                                with open("cover.jpg", "rb") as af: audio.tags.add(APIC(encoding=3, mime='image/jpeg', type=3, desc='Cover', data=af.read()))
                            audio.save(v2_version=3, v1=2)
                            shutil.move("temp_raw.mp3", os.path.join(target_dir, clean_en + ".mp3"))
                            m3u8_lines.append(f"\\MUSIC\\{clean_en}.mp3")
                        except Exception as e: self.root.after(0, lambda err=str(e): self.write_log(f"  ✕ Entry failed: {err[:40]}"))
                    if m3u8_lines:
                        with open(os.path.join(playlist_dir, "".join(x for x in item['title'] if x.isalnum() or x in " .-_")[:100] + ".m3u8"), "w", encoding="utf-8") as f: f.write("\n".join(m3u8_lines))
                    item['status'] = 'success'
                    ...
                    if item.get('is_local'):
                        ...
                        opts = {'format': 'bestaudio/best', 'ffmpeg_location': ff_path, 'outtmpl': 'temp_raw.%(ext)s', 'nopart': True, 'continuedl': False, 'postprocessors': [{'key': 'FFmpegExtractAudio', 'preferredcodec': 'mp3', 'preferredquality': '192'}], 'postprocessor_args': ['-map_metadata', '-1']}
                        if self.selected_browser and self.selected_browser != "None":
                            try: opts['cookiesfrombrowser'] = (self.selected_browser.lower(),)
                            except: pass
                        with yt_dlp.YoutubeDL(opts) as ydl: info_dict = ydl.extract_info(item['url'], download=True)
                    ...
                    os.makedirs(target_dir, exist_ok=True)
                    if item.get('is_local'):
                        subprocess.run([ff_path, "-y", "-i", item['url'], "-c:v", "libx264", "-profile:v", "baseline", "-level", "3.0", "-pix_fmt", "yuv420p", "-vf", "scale=480:272", "-b:v", "768k", "-c:a", "aac", "-ac", "2", "-b:a", "128k", "-ar", "48000", "temp.mp4"], startupinfo=startupinfo)
                        try:
                            subprocess.run([ff_path, "-y", "-i", item['url'], "-ss", "00:00:01", "-vframes", "1", "temp_thumb.jpg"], startupinfo=startupinfo)
                            if os.path.exists("temp_thumb.jpg"): img = Image.open("temp_thumb.jpg")
                        except: pass
                    else:
                        opts = {'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best', 'ffmpeg_location': ff_path, 'outtmpl': 'temp_raw.%(ext)s', 'nopart': True, 'continuedl': False}
                        if self.selected_browser and self.selected_browser != "None":
                            try: opts['cookiesfrombrowser'] = (self.selected_browser.lower(),)
                            except: pass
                        with yt_dlp.YoutubeDL(opts) as ydl: info_dict = ydl.extract_info(item['url'], download=True)
                        img = self._get_best_thumbnail(info_dict, item.get('thumb'))
                        raw_file = next((f for f in os.listdir(".") if f.startswith("temp_raw.") and not f.endswith(".mp3")), None)
                        if raw_file:
                            self.root.after(0, lambda: self.write_log("Converting to PSP format (H.264 480×272)…"))
                            subprocess.run([ff_path, "-y", "-i", raw_file, "-c:v", "libx264", "-profile:v", "baseline", "-level", "3.0", "-pix_fmt", "yuv420p", "-vf", "scale=480:272", "-b:v", "768k", "-c:a", "aac", "-ac", "2", "-b:a", "128k", "-ar", "48000", "temp.mp4"], startupinfo=startupinfo)
                    if 'img' in locals() and img:
                        try:
                            w2, h2 = img.size; target_ratio = 160 / 120.0
                            if w2 / h2 > target_ratio: img = img.crop(((w2-int(target_ratio * h2))//2, 0, (w2+int(target_ratio * h2))//2, h2))
                            else: img = img.crop((0, (h2-int(w2 / target_ratio))//2, w2, (h2+int(w2 / target_ratio))//2))
                            img.resize((160, 120), Image.Resampling.LANCZOS).convert('RGB').save(os.path.join(target_dir, clean_name + ".thm"), "JPEG")
                        except Exception as e: self.root.after(0, lambda err=str(e): self.write_log(f".THM Error: {err[:40]}"))
        ...
        if sys.platform == "darwin":
            try: subprocess.run(["dot_clean", "-fm", drive_path], capture_output=True)
            except: pass

        self.is_processing = False
        self.root.after(0, lambda: self.update_progress(100))
        self.root.after(0, lambda: self._set_send_btn("⬆  SEND QUEUE VIA USB", "normal"))
    ...
    def _get_best_thumbnail(self, info_dict, fallback_url):
        ...
        if info_dict and info_dict.get('thumbnail'): urls.append(info_dict['thumbnail'])
